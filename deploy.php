 ...
 * Usage: ?token=<token from chefing.cfg>
 */

// ── Config ────────────────────────────────────────────────────────────────────
$ini      = file_exists(__DIR__ . '/chefing.cfg') ? parse_ini_file(__DIR__ . '/chefing.cfg', true) : [];
$u        = $ini['upgrade'] ?? [];
$token    = $u['token']  ?? '';
$branch   = $u['branch'] ?? 'main';
$repo     = $u['repo']   ?? 'PluggPreagar/chefing';
...
// Detect top-level prefix, e.g. "chefing-main/" and read commit metadata
$prefix      = '';
