...
| T26 | Kaskaden-Zielrolle + Ersatzkandidat (Schritt 2/3) | open | Neues optionales Feld an `voraussetzung`-Einträgen in `operations.json`: `kaskade_ziel` (Rolle, die bei Kollision angepasst werden könnte, z. B. bei der Natron-Säure-Regel → `fluessigkeit`). `rules.js`/`engine.js`: wenn eine `hart`-Voraussetzung mit `kaskade_ziel` fehlschlägt und die Zielrolle nicht `gesetzt-fix` ist, ersten passenden Ersatzkandidaten aus `rolle.optionen` der Zielrolle ermitteln (Kandidat erfüllt die geforderte Eigenschaft) und als Vorschlag in `computed` bereitstellen — noch keine State-Mutation, nur Berechnung/Datenstruktur. | T24 |
| T27 | Vorschlagen-dann-bestätigen-UI für Kaskade (Schritt 3/3) | open | `morphbox.js`/`recipeCard.js`: den in T26 berechneten Ersatzvorschlag anzeigen („Buttermilch statt Milch verwenden?") mit Bestätigen-Button, der `state.zutaten` erst nach Klick mutiert (kein stilles Umschalten, DR-019 Punkt 1 Vorbild: Vorschlagen-dann-bestätigen wie bei der Erfahrungs-Ebene). Damit ist die „Kaskaden-Propagation" aus DR-019 vollständig umgesetzt. | T26 |
