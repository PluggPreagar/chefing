...
| T32 | Generische Korpus-Korrelationssuche (DR-019 Punkt 5) | open | Korpus generisch nach bedingten Abhängigkeiten durchsuchen (Median/IQR einer Rolle bedingt auf eine Eigenschaft einer anderen Rolle), automatischer Vorschlag + einmalige Nutzer-Bestätigung, bevor eine Regel aktiv wird (Vorstufe war T23 — statische Erfahrungs-Regeln in Daten, dieser Schritt ist die automatische Ableitung neuer Regeln aus dem Korpus). Braucht einen größeren Korpus als heute (~7 Rezepte/Variante, DR-003-Risiko „Ausreißer verzerren IQR stark“). | T04, T23 |
